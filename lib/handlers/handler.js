...
async function parseSymbolsFile (fileStream, logger = logger, args = []) {
    ...
    symbols.reduce((prevId, word) => {
        let chapter = parseInt(prevId.substring(4, 6));
        let verse = parseInt(prevId.substring(7, 9));
        let wordCount = parseInt(prevId.substring(10, 11));
        if (word.chapter !== null) {
        ...
        word._id = `1${bookNum}${chapter.toString().padStart(3, '0')}${verse.toString().padStart(3, '0')}${(++wordCount).toString().padStart(2, '0')}`;
        return word._id;
    }, `1${bookNum}00000000`);
    //await mongo.getWordsCollection().insertMany(words);
}
