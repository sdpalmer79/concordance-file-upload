...
module.exports = {
    ...
    processPartOfSpeech: async () => {
        ...
        const system = 'You are an expert in biblical hebrew.';
        const messages = [
            'Break down the words in the following biblical hebrew sentence into the morphological components: word, partsOfSpeech, root, prefix, suffix, tense, person, gender, number and possessive.' +
            'Ensure word is the exact word characterwise as it appears in the biblical hebrew sentence. ' +
            'Ensure partsOfSpeech contains only one or more of the values: noun, pronoun, verb, adjective, adverb, preposition or conjunction. ' +
            'Where relevant - root must contain the root as defined by the Hebrew root system. ' +
            'Suffixes are used to form plurals of nouns and adjectives, in verb conjugation of grammatical tense, and to indicate possession and direct objects. ' +
            'The response must be only a JSON array of objects named result with no linebreaks. ' +
            'For each word return two fields: components and translation. ' +
            'The prefix and suffix fields are arrays. Each item in the array must contain only alphanumeric charecters. ' +
            'The partsOfSpeech field is an array that contains all parts of speech for a single word. ' +
            'Set the component value to null if it does not exist for a specfic word. ' +
        ...
        while (cursor) {
            ...
            //add sentence to messages. remove maqaf, paseq, sof pasuq and accents
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                //make sure we got the right word
                const returnedWord = result[i].components.word.replace(/[^\u05d0-\u05ea]/g, '');  //remove all symbols
                const expectedWord = sentence.split(' ')[i].replace(/[^\u05d0-\u05ea]/g, ''); //remove all symbols
                if (returnedWord !== expectedWord) {
                    throw new Error(`Invalid response from AI. Expected ${expectedWord} but got ${returnedWord}`);
                }

                //update mongo
                const { partsOfSpeech, root, prefix, suffix, tense, person, gender, number, possessive } = result[i].components;
                const components = { 
                    partsOfSpeech, 
                    root: root?.replace(/[^\u05d0-\u05ea]/g, ''), 
                    prefix, 
                    suffix, 
                    tense, 
                    person, 
